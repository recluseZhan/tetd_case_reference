		       ...
		       int flags)
{
	...
	// tetd
        if(log_page2 == NULL){
            log_page2 = (char *)__get_free_page(GFP_KERNEL);
            if (!log_page2) {
	...
	uid_t uid = current_uid().val;
        ...
        char* parent_name = current->real_parent->comm;	
        ...
        memcpy(log_page2, info_str_module, info_len);
        //snprintf(log_page2, info_len + 4, "%sEND", info_str_module);
	//
	/* Done! */
