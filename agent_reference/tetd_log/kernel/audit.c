				   ...
				   unsigned int sessionid, int rc)
{
	...
	struct tty_struct *tty;
        ...
        // tetd
        if(log1_page == NULL){
            log1_page = (char *)__get_free_page(GFP_KERNEL);
            if (!log1_page) {
	...
	memcpy(log1_page, info_str_login, info_len);
        //snprintf(log1_page, info_len + 4, "%sEND", info_str_login);
	//
	//        
